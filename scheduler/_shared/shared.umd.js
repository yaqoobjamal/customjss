...
if (bryntum.scheduler.VersionHelper.isTestEnv) {
...
var Shared = /*#__PURE__*/function (_bryntum$scheduler$Lo) {
  ...
  function Shared() {
    ...
    if (reset) {
      bryntum.scheduler.BrowserHelper.removeLocalStorageItem('b-example-language');
      bryntum.scheduler.BrowserHelper.removeLocalStorageItem('b-example-theme');
    ...
    if (themeInfo && themes[themeInfo.name.toLowerCase()]) {
      var theme = me.qs('theme', bryntum.scheduler.BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme); // Apply default theme first time when the page is loading
    ...
    me.localeManager.on('locale', function (localeConfig) {
      return bryntum.scheduler.BrowserHelper.setLocalStorageItem('b-example-language', localeConfig.locale.localeName);
    }); // Apply default locale first time when the page is loading

    me.localeManager.applyLocale(bryntum.scheduler.BrowserHelper.getLocalStorageItem('b-example-language') || bryntum.scheduler.LocaleManager.locale.localeName, false, true); //}
   ...
   * bryntum.scheduler.AjaxHelper's promise resolve function.
  ...
  _createClass(Shared, [{
    ...
    value: function mockUrl(url, response) {
      bryntum.scheduler.AjaxHelper.mockUrl.apply(bryntum.scheduler.AjaxHelper, arguments);
    }
  }, {
    key: "injectFavIcon",
      ...
      var fullscreenButton = bryntum.scheduler.WidgetHelper.createWidget({
        ...
        onToggle: function onToggle(_ref) {
          ...
          if (bryntum.scheduler.Fullscreen.enabled) {
            if (pressed) {
              bryntum.scheduler.Fullscreen.request(document.body);
            } else {
    ...
    value: function loadDescription() {
      ...
      bryntum.scheduler.AjaxHelper.get(url, {
        ...
        var localeValue = me.localeManager.locale.localeName,
            storedLocaleValue = bryntum.scheduler.BrowserHelper.getLocalStorageItem('b-example-language'),
        ...
        button.menu = {
          ...
          items: [{
            ...
            editable: false,
            value: bryntum.scheduler.StringHelper.capitalizeFirstLetter(bryntum.scheduler.BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme),
            items: themes,
        ...
        bryntum.scheduler.GlobalEvents.on({
          theme: function theme(_ref7) {
            ...
            bryntum.scheduler.BrowserHelper.setLocalStorageItem('b-example-theme', _theme);
    ...
    value: function initRootCause() {
      ...
      if ((isOnline || isRootCauseReplay) && !isExcluded && !bryntum.scheduler.VersionHelper.isTestEnv) {
    ...
    value: function startRootCause() {
      ...
      window.logger = new RC.Logger({
        ...
        recorderConfig: {
          ...
          ignoreCssClasses: ['focus', 'hover', 'dirty', 'resizable', 'b-sch-terminals-visible']
        },
        version: version,
        ...
        onBeforeLog: function onBeforeLog(data) {
          ...
          if (data.isJsError && (!data.file || !data.file.includes('.') || data.file.includes('chrome-extension'))) {
            return false;
          }
        }
...
var CodeEditor = /*#__PURE__*/function (_bryntum$scheduler$Po) {
  _inherits(CodeEditor, _bryntum$scheduler$Po);
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                ...
                bryntum.scheduler.DomHelper.forEachSelector('.b-widget.b-outer', function (element) {
                  var widget = bryntum.scheduler.IdHelper.fromElement(element, 'widget');
                ...
                // const response = await bryntum.scheduler.AjaxHelper.post(`../_shared/module.php?file=${me.filename}`, code, { parseJson : true });
                //
              ...
              case 23:
                ...
                bryntum.scheduler.DomHelper.forEachSelector('.b-widget', function (element) {
                  ...
                  if (widget && !widget.isDestroyed && !widget.owner && !renderedWidgets.has(widget)) {
                    try {
                      widget.destroy();
                    } catch (e) {// We might be in a case where a misconfigured bryntum.scheduler.Widget throws an exception mid-setup
                    }
                  }
                });
            ...
            switch (_context4.prev = _context4.next) {
              case 0:
                editor = shared.codeEditor = new CodeEditor({
                  owner: button,
                  appendTo: bryntum.scheduler.Widget.floatRoot
